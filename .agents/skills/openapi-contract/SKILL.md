...
6. Add or update semantic assertions in `api-docs/embed_test.go` for cross-cutting contract invariants.
...
9. Run `just build`, `just test`, and `just lint`. Use `just check-all` when shared tooling or the separate module is in
   scope.
...
- Every operation ID is unique and every route matches registration.
